...
class UserDatabase extends Database
{
    ...
    public static $USER_FATHER_FIRST = "father_first";
    public static $USER_MOTHER_FIRST = "mother_first";
    public static $USER_MOTHER_MAIDEN = "mother_maiden";
    public static $USER_ADD_DATETIME = "add_datetime";
    public static $USER_PHONE_NUMBER = "phone_number";
    public static $USER_IS_ACTIVE = "is_active";
    ...
    public function editUserByUser($first_name, $last_name, $e_mail, $phone_number, $tc,
                                   $father_first, $mother_first, $mother_maiden, $user_id_from_session)
    {
        $sql = sprintf("
            ...
            self::$USER_PHONE_NUMBER,
            self::$USER_TC,
            self::$USER_FATHER_FIRST,
            self::$USER_MOTHER_FIRST,
            self::$USER_MOTHER_MAIDEN,
            self::$USER_ID
        );

        if ($stmt = $this->getDb()->prepare($sql)) {
            $stmt->bind_param("ssssssssd", $first_name, $last_name, $e_mail, $phone_number,
                $tc, $father_first, $mother_first, $mother_maiden, $user_id_from_session);
            $res = $stmt->execute();
            if (!$res) {
                $this->setIsError(true);
                $this->setErrorMessage("Kullanıcı düzenleme sırasında bir hata meydana geldi : " . $this->getDb()->error);
    ...
    public function getCustomerName($user_id)
    {
        if ($user_id == null) {
            ...
            if ($stmt = $this->getDb()->prepare($sql)) {
                ...
                return $singleRow['first_name'] . " " . $singleRow['last_name'];
            } else {
                $this->setErrorMessage("Id için kullanıcı bilgileri getirmede sıkıntı meydana geldi  : " . $this->getDb()->error);
                return null;
            }
    ...
    public function create_user($email, $password, $firstName, $lastName, $motherName, $fatherName, $maidenName, $tc, $tel): bool
    {
        ...
        if ($stmt = $this->getDb()->prepare($sql)) {
            ...
            $stmt->bind_param("sssssssss", $email, $md5_password, $firstName, $lastName, $fatherName, $motherName, $maidenName, $tc, $tel);
