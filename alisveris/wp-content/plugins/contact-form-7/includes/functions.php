...
function wpcf7_deprecated_function( $function, $version, $replacement ) {
	...
	if ( WP_DEBUG && $trigger_error ) {
		if ( function_exists( '__' ) ) {
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since Contact.class Form 7 version %2$s! Use %3$s instead.', 'contact-form-7' ), $function, $version, $replacement ) );
		} else {
			trigger_error( sprintf( '%1$s is <strong>deprecated</strong> since Contact.class Form 7 version %2$s! Use %3$s instead.', $function, $version, $replacement ) );
		}
	}
}
