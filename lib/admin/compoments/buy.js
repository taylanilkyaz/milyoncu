;(function ($, window, document, undefined) {
  ...
  $("#filtrele-ara").click(function () {
    var smallval = $("#smallval").val();
    var largeval = $("#largeval").val();
    $.ajax({
      method: 'POST',
      url: 'buy-form.php',
      data: {smallval : smallval , largeval:largeval}
    }).done(function (msg) {
    })
  ...
  function insertBasketInfo (header) {
    $.uiAlert({
      textHead: '', // header
      text: '', // Text
      bgcolor: '#0ff204', // background-color
      textcolor: '#fff', // color
      position: 'bottom-right',// position . top And bottom ||  left / center / right
      icon: 'checkmark icon', // icon in semantic-UI
      time: 1, // time
    })
  }
