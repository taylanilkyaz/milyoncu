...
//	Try our best to set base path here just in case it wasn't set in ajax.php (pre version 1.0.1)
		...
		function qa_index_set_request()
		{
			...
			elseif (isset($_GET['qa'])) {
				if (strpos($_GET['qa'], '/') === false) {
					$urlformat = ( empty($_SERVER['REQUEST_URI']) || strpos($_SERVER['REQUEST_URI'], '/ajax.php') !== false )
